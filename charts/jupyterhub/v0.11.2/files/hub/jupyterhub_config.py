...
if get_config("gpu.enabled", False):
    gpu_type = get_config("gpu.type")
    gpu_amount = get_config("gpu.amount")
    node_selector = dict()
    if gpu_type == "any":
        node_selector = dict(
            matchExpressions=[
    ...
    c.KubeSpawner.node_affinity_required.append(node_selector)
    c.KubeSpawner.extra_resource_guarantees = {"nvidia.com/gpu": gpu_amount}
    c.KubeSpawner.extra_resource_limits = {"nvidia.com/gpu": gpu_amount}

admin_user = get_config("adminUsername")
c.JupyterHub.Authenticator.admin_users = [admin_user]
#allowed_users = get_config("allowedUsers")
#c.JupyterHub.Authenticator.allowed_users = allowed_users.split('\n')
...
if get_config("singleuser.storage.sharedStorageEnabled", False):
    sharedStorageClaimName = get_config("singleuser.storage.sharedStorageClaimName")
    sharedStorageMountPath = get_config("singleuser.storage.sharedStorageMountPath")
    c.KubeSpawner.volumes.extend([{
        "name": "shared",
        "persistentVolumeClaim": {
            "claimName": sharedStorageClaimName,
            "storageAccessModes": ["ReadWriteMany"],
        },
    }])
    c.KubeSpawner.volume_mounts.extend([{
        "mountPath": sharedStorageMountPath,
        "name": "shared",
    }])
